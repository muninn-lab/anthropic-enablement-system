...
The official marketplace includes several categories of plugins, such as: code intelligence, external integrations, development workflows and output styles.
...
| `source` | string\|object | Where to fetch the plugin from (plugin sources)                                                                           |
...
<Note>
  Relative paths only work when users add your marketplace via Git (GitHub, GitLab, or git URL). If users add your marketplace via a direct URL to the `marketplace.json` file, relative paths will not resolve correctly. For URL-based distribution, use GitHub, npm, or git URL sources instead. 
</Note>
...
# Demo: create a local marketplace
